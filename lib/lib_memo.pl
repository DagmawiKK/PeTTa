...
% Key Canonicalization & Answer Replay

memoization_enabled_for_call(Fun, CallArity) :-
    memo_enabled(Fun)
    ; memo_enabled(Fun, CallArity).

% Check if predicate arity (arity + 1) has memoization enabled for call arity.
...
% Check if function is defined, memoized, and not built-in.
...
% Round float to configured precision to handle rounding-error normalization.
quantize_float(V, Q) :-
    memo_float_precision(Prec),
    Scale is 10.0 ** Prec,
    Q is round(V * Scale) / Scale.

% Recursively quantize floats in term; preserve variables and atoms.
...
% Check if arguments are too large relative to memory limit.
...
% Check if arguments are worth caching (not too large).
...
% Unify variant answer: check input bindings match, then unify output.
replay_variant_answer(AVs, Out, answer(CachedAVs, CachedOut)) :-
    AVs = CachedAVs,
    Out = CachedOut.

% Unify ground answer: just unify output.
replay_ground_answer(Out, answer(CachedOut)) :-
    Out = CachedOut.

% Synchronization Helpers

% Derive mutex ID from (Fun, Arity) pair.
cache_fun_mutex_id(Fun, Arity, Mutex) :-
    atomic_list_concat(['metta_cache_fun_', Fun, '_', Arity], Mutex).

% Execute goal under (Fun, Arity)-specific mutex for thread safety.
with_cache_fun_mutex(Fun, Arity, Goal) :-
    cache_fun_mutex_id(Fun, Arity, Mutex),
    with_mutex(Mutex, Goal).

% Execute goal under CMS global mutex for thread safety.
with_cms_mutex(Goal) :-
    with_mutex(metta_cache_cms, Goal).

% Frequency Sketch (CMS / WTinyLFU)

% Initialize Count-Min Sketch if not already present.
ensure_cms :-
...
% Compute CMS slot for cache key; return structure, hash, and sketch size.
cms_slot(Fun, Arity, AVs, CMS, Hash, SketchSize) :-
...
% Get frequency estimate from CMS.
get_freq(Fun, Arity, AVs, Freq) :-
    with_cms_mutex(
        ( cms_slot(Fun, Arity, AVs, CMS, Hash, _)
        -> arg(Hash, CMS, Val),
           ( integer(Val) -> Freq = Val ; Freq = 0 )
        ; Freq = 0 )
    ).

% Record cache hit by incrementing CMS frequency.
...
% Record cache miss by incrementing frequency and access counter.
...
% Decay all CMS frequencies by halving them (periodic reset).
halve_cms :-
    nb_setval(metta_memo_accesses, 0),
...
% Update queue state counters (count, head, tail).
...
% Recursive eviction with attempt counter and safety cap.
...
% Insert entry into cache queue at next tail position.
...
% Main store dispatcher: enforce limit and invoke eviction policy (LRU or WTinyLFU).
...
% Evict victim and admit new entry; update bytes and queue state.
...
% Store only if generation is still current (else entry was invalidated).
...
% Apply configured aggregation mode (none, count, sum, min, max) to answers.
...
% No aggregation — return all answers.
apply_aggregate_mode(none, ProbeResults, ProbeResults).
% Count aggregation — return count of answers.
apply_aggregate_mode(count, ProbeResults, [answer(Count)]) :-
    length(ProbeResults, Count).
% Sum/min/max aggregation — apply list predicate to extracted values.
...
% Truncate answer list to configured limit.
...
% In-Progress Guards & Waiting

% Mark key as in-progress to prevent recursive recomputation.
...
% Call Context & Dependency Tracking

% Execute goal with call context for dependency tracking.
...
% Lookup cached entry in current generation.
cache_lookup(Fun, Arity, CurGen, KeyAVs, CachedResults) :-
    metta_memo_entry(Fun, Arity, CurGen, KeyAVs, CachedResults).

% Replay ground hit by selecting one cached answer.
...
% Replay variant hit by unifying with a cached answer.
...
% Truncate answers to limit and store; record miss in frequency sketch.
...
% Probe variant answers and store results.
...
% Store ground path: probe, apply aggregation, cache results.
...
% Store variant path: manage in-progress guard, probe, cache, or wait/fallback.
...
% Main cache dispatch: check ground/variant paths, lookup/store.
...
% Find all functions that transitively depend on SeedFun (reverse dependency closure).
impacted_functions(SeedFun, Impacted) :-
    impacted_functions([SeedFun], [], Raw),
    sort(Raw, Impacted).

% Accumulator version of impacted_functions with cycle detection.
impacted_functions([], Seen, Seen).
impacted_functions([Fun|Rest], Seen, Impacted) :-
    ( ord_memberchk(Fun, Seen)
    -> impacted_functions(Rest, Seen, Impacted)
    ; findall(Caller, metta_memo_dep(Caller, _, Fun, _), Callers),
      append(Rest, Callers, Next),
      ord_add_element(Seen, Fun, NewSeen),
      impacted_functions(Next, NewSeen, Impacted)
    ).

% Invalidate all cache entries for Fun at all arities.
...
% Wipe all cache state: entries, generations, queues, dependencies, stats, and CMS.
cache_clear :-
